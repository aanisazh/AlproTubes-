...
func TranskipNilai(data *TabMahasiswa, jumlahData int) {
	...
	for i := 0; i < jumlahData; i++ {
		fmt.Printf("%-10s %-20s %-15s ", data[i].nim, data[i].nama, data[i].jurusan)
		for j := 0; j < len(data[i].matkul); j++ {
			total := data[i].matkul[j].quiz + data[i].matkul[j].uts + data[i].matkul[j].uas
			grade := calculateGrade(total)
			fmt.Printf("%-6d %-6d %-6d %-6d %-6s\n", data[i].matkul[j].quiz, data[i].matkul[j].uts, data[i].matkul[j].uas, total, grade)
		}
	}
}
